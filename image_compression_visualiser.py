...
VIZ_SAMPLE_SIZE = 25000 # Increased for better 3D plot density
...
def calculate_metrics(original_img_arr, compressed_img_arr):
    ...
    ssim = structural_similarity(original_img_arr, compressed_img_arr, channel_axis=-1, data_range=255)
    return {'MSE': mse, 'PSNR': psnr, 'SSIM': ssim}

def plot_2d_hsv_difference(original_rgb, compressed_rgb, chart_type, output_path):
    """(Previously plot_hsv_difference) Generates 2D static plots of HSV differences."""
    hsv_orig = skimage.color.rgb2hsv(original_rgb)
    hsv_comp = skimage.color.rgb2hsv(compressed_rgb)
    
    # Flatten arrays and subsample
    ...
    delta_h = h_comp - h_orig; delta_h[delta_h > 0.5] -= 1.0; delta_h[delta_h < -0.5] += 1.0
    ...
    chart_params = {
        'sv_vs_h_delta': (v_orig, s_orig, delta_h, 'Saturation vs. Brightness (Colored by Hue Change)', 'Original Brightness (Value)', 'Original Saturation', 'Hue Change (ΔH)', 'twilight_shifted', (-0.1, 0.1)),
        'hs_vs_v_delta': (s_orig, h_orig, delta_v, 'Hue vs. Saturation (Colored by Brightness Change)', 'Original Saturation', 'Original Hue', 'Brightness Change (ΔV)', 'coolwarm', (-0.2, 0.2)),
        'hv_vs_s_delta': (v_orig, h_orig, delta_s, 'Hue vs. Brightness (Colored by Saturation Change)', 'Original Brightness (Value)', 'Original Hue', 'Saturation Change (ΔS)', 'coolwarm', (-0.2, 0.2)),
    }
    x, y, c, title, xlabel, ylabel, clabel, cmap, clim = chart_params[chart_type]
    
    scatter = ax.scatter(x, y, c=c, cmap=cmap, s=10, alpha=0.6, edgecolors='none')
    cbar = fig.colorbar(scatter, ax=ax); cbar.set_label(clabel, fontsize=12); scatter.set_clim(clim)
    ax.set_title(title, fontsize=16, pad=20); ax.set_xlabel(xlabel, fontsize=12); ax.set_ylabel(ylabel, fontsize=12)
    if 'Hue' in ylabel: ax.set_ylim(0, 1)
...
def plot_3d_interactive(rgb_orig, rgb_comp, plot_type, output_path):
    ...
    - 'distribution': Shows the original image's pixel distribution in an HSV cylinder.
    - 'difference': Shows the HSV delta (error) between original and compressed.
    """
    print(f"Generating 3D plot ({plot_type})... This might take a moment.")
    
    # --- Data Preparation ---
    # Flatten and subsample original RGB data
    ...
    # Convert sampled original RGB to HSV
    hsv_orig = skimage.color.rgb2hsv(sampled_rgb_orig.reshape(-1, 1, 3)).reshape(-1, 3)
    h_orig, s_orig, v_orig = hsv_orig.T
    
    # Prepare marker colors (Plotly needs 'rgb(r,g,b)' strings)
    marker_colors = [f'rgb({r},{g},{b})' for r, g, b in sampled_rgb_orig]

    fig = go.Figure()

    if plot_type == 'distribution':
        # --- Original Color Distribution Plot (HSV Cylinder) ---
        # Convert cylindrical HSV to Cartesian coordinates for plotting
        x = s_orig * np.cos(h_orig * 2 * np.pi)
        y = s_orig * np.sin(h_orig * 2 * np.pi)
        z = v_orig # Value becomes the Z axis
        ...
        # --- Color Difference Plot (HSV Deltas) ---
        # Get corresponding pixels from compressed image and convert to HSV
        flat_rgb_comp = rgb_comp.reshape(-1, 3)
        sampled_rgb_comp = flat_rgb_comp[indices]
        hsv_comp = skimage.color.rgb2hsv(sampled_rgb_comp.reshape(-1, 1, 3)).reshape(-1, 3)
        ...
        fig.add_trace(go.Scatter3d(
            x=delta_s, y=delta_v, z=delta_h,
            mode='markers',
            marker=dict(size=3, color=marker_colors, opacity=0.7), # Color by ORIGINAL color
            hovertext=[f"Original H:{h:.2f}, S:{s:.2f}, V:{v:.2f}<br>ΔS:{ds:.3f}, ΔV:{dv:.3f}, ΔH:{dh:.3f}" 
                       for h,s,v,ds,dv,dh in zip(h_orig, s_orig, v_orig, delta_s, delta_v, delta_h)],
            hoverinfo='text'
        ))
        
        fig.update_layout(
            title='Interactive 3D Color Difference Plot (Colored by Original Pixel)',
            scene=dict(
...
Generates 2D static charts and 3D interactive HTML plots.""",
    ...
    parser.add_argument('original_image', help="Path to the original, high-quality image.")
    parser.add_argument('compressed_images', nargs='*', help="Path(s) to compressed/modified images. If none, only analyzes the original.")
    
    args = parser.parse_args()

    if not os.path.exists(args.original_image):
        print(f"Error: Original image not found at '{args.original_image}'"); return
    ...
    try:
        ...
        print(f"Error loading original image '{args.original_image}': {e}"); return

    # --- 1. Analyze Original Image Distribution ---
    print("\n--- Analyzing Original Image ---")
    orig_basename = os.path.splitext(os.path.basename(args.original_image))[0]
    plot_3d_interactive(original_array, None, 'distribution',
    ...
    # --- 2. Compare Against Compressed Versions ---
    ...
    for compressed_path in args.compressed_images:
        if not os.path.exists(compressed_path):
            print(f"Warning: Skipping non-existent file '{compressed_path}'"); continue
        try:
            ...
            print(f"{filename:<25} | {metrics['MSE']:<12.2f} | {metrics['PSNR']:<12.2f} | {metrics['SSIM']:<12.4f}")

            base_filename = os.path.splitext(filename)[0]
            
            # Generate 2D charts
            plot_2d_hsv_difference(original_array, compressed_array, 'sv_vs_h_delta',
                                os.path.join(OUTPUT_DIR, f"{base_filename}_2d_SvV_HueDelta.png"))
            plot_2d_hsv_difference(original_array, compressed_array, 'hs_vs_v_delta',
                                os.path.join(OUTPUT_DIR, f"{base_filename}_2d_HvS_ValDelta.png"))
            plot_2d_hsv_difference(original_array, compressed_array, 'hv_vs_s_delta',
                                os.path.join(OUTPUT_DIR, f"{base_filename}_2d_HvV_SatDelta.png"))
            
            # Generate 3D interactive chart
            plot_3d_interactive(original_array, compressed_array, 'difference',
                                os.path.join(OUTPUT_DIR, f"{base_filename}_3d_difference.html"))
            print("-" * 70)

        except ValueError as e: print(f"Skipping '{compressed_path}': {e}")
        except Exception as e: print(f"Could not process file '{compressed_path}': {e}")
